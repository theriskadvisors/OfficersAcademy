//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SEA_Application.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class AspNetStudentPerformanceReport
    {
        public int Id { get; set; }
        public string StudentID { get; set; }
        public Nullable<int> SubjectID { get; set; }
        public string Title { get; set; }
        public string Description { get; set; }
        public string LearningConcept { get; set; }
        public string TimeManagement { get; set; }
        public string Homework { get; set; }
        public string ReadingSkills { get; set; }
        public string InstructionFollowing { get; set; }
        public string ProjectLines { get; set; }
        public string Handwriting { get; set; }
        public string PerformanceSkills { get; set; }
        public string Punctuality { get; set; }
        public string Regularity { get; set; }
        public string Assessment { get; set; }
        public Nullable<System.DateTime> Date { get; set; }
    
        public virtual AspNetSubject AspNetSubject { get; set; }
        public virtual AspNetUser AspNetUser { get; set; }
    }
}
