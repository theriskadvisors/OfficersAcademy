...
namespace SEA_Application.Controllers
{
    ...
    public class ManageController : Controller
    {
       ...
       // [ValidateAntiForgeryToken]
        ...
        public ActionResult username_update(ProfileData Profdata)
        {
            string status = "error";
          int emailcounter = db.AspNetUsers.Where(x=>x.Email  == Profdata.emailaddress).Count();
            AspNetUser u = db.AspNetUsers.FirstOrDefault(x => x.Id == Profdata.UserId);
            u.Name = Profdata.username;
            u.PhoneNumber = Profdata.phoneno;
            u.Email = Profdata.emailaddress;
            if (db.SaveChanges() > 0 && emailcounter <=1)
            {
               status = "success";
            }
